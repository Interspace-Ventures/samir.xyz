...
- **Deployment Build Failure Fixed (security scan)**: Publish was failing at the security-scan step. Replit's scanner (gitleaks) flags the auto-generated `previewModeSigningKey` / `previewModeEncryptionKey` inside `.next/prerender-manifest.json` as secrets. These are throwaway keys that `next build` regenerates on every deploy, so removing `.next` from git (or scrubbing history) does NOT fix it — the deploy rebuilds the file each time and the scan reads the build output. Fix: a post-build step (`scripts/strip-preview-keys.mjs`) blanks those preview keys right after `next build`, before the scan. Wired into the deployment build command: `npx prisma generate && npx next build && node scripts/strip-preview-keys.mjs`. Safe because the site doesn't use Next.js Preview/Draft Mode.
